    ...
    <div className={`${mobile ? "mt-2" : "absolute mt-2 w-full z-50"} bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-fadeInScale`}>
      {results.map((r) => (
        <Link
          ...
          className="flex items-center gap-3 p-2.5 hover:bg-slate-50 transition-colors text-right"
        >
      ...
      <button
        onClick={goToSearch}
        className="w-full flex items-center justify-center gap-1.5 text-xs font-bold text-brand py-2.5 hover:bg-slate-50 border-t border-slate-100 transition-colors"
      >
  ...
  return (
    <header
      className={`sticky top-0 z-40 bg-white/95 backdrop-blur border-b transition-shadow duration-300 ${
        scrolled ? "shadow-soft border-slate-100" : "border-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center gap-3 py-3">
          <button
            className="lg:hidden p-2 -ms-2 text-brand hover:bg-slate-50 rounded-lg transition-colors"
            onClick={onOpenMenu}
            aria-label="القائمة"
          >
            <Menu size={24} />
          ...
          <button
            className="md:hidden p-2 text-brand hover:bg-slate-50 rounded-lg transition-colors"
          ...
          <Link
            to="/cart"
            className="relative p-2.5 bg-brand rounded-xl text-white hover:bg-brand-light active:scale-95 transition-all"
          >
